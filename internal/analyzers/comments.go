...
const (
	cgoGeneratedComment   = "// Code generated by cmd/cgo; DO NOT EDIT."
	maxLineDirectiveValue = 1 << 30
)
...
func isBlockLineDirective(text string) bool {
	...
	body, hasSuffix := strings.CutSuffix(body, "*/")
	isSingleLine := !strings.ContainsAny(body, "\r\n")
	hasValidPosition := hasValidLineDirectivePosition(body)
	isValidDirective := hasSuffix && isSingleLine && hasValidPosition
	return isValidDirective
}

func hasValidLineDirectivePosition(body string) bool {
	prefix, final, hasFinal := trailingDirectiveNumber(body)
	hasValidFinal := hasFinal && isValidLineDirectiveValue(final)
	if !hasValidFinal {
		return false
	}

	_, line, hasColumn := trailingDirectiveNumber(prefix)
	hasValidLine := isValidLineDirectiveValue(line)
	hasValidPosition := !hasColumn || hasValidLine
	return hasValidPosition
}

func trailingDirectiveNumber(value string) (string, uint64, bool) {
	...
	return value[:separator], number, err == nil
}

func isValidLineDirectiveValue(value uint64) bool {
	isPositive := value > 0
	isWithinRange := value <= maxLineDirectiveValue
	return isPositive && isWithinRange
}
